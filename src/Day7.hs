...
removeNode n = do
  ...
  where
    ...
    subtractIncoming n = do
        mc <- gets (M.lookup n . incoming)
        case mc of
            Just c -> do
                modify
                    (\ts@TopSort {..} ->
                         ts {incoming = M.insert n (c - 1) incoming})
                when (c == 1) $
                    modify (\ts@TopSort {..} -> ts {next = sort (n : next)})
    ...
    assignNext = do
        ...
        case nxt of
            (n:ns) ->
                modify
                    (\ts@TopSort {..} ->
                         ts {next = ns, workers = assign n workers})
    assign n ws =
